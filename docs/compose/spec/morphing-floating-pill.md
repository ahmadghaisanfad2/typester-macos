...
**Verification** — `swift build` PASS; `swift test` PASS (319 tests, 0 failures) including 7 `DockReserveTests` and 10 `PillAnchorPolicyTests`. Debug harness `TYPESTER_DEMO=pill TYPESTER_PILL_MODE=collapsed|live TYPESTER_SNAPSHOT=…` renders each state; the reposition log confirms the shared anchor. AppKit rendering, morph smoothness, hover actions and Dock clearance remain residual manual checks (the snapshot harness renders blank without an attached WindowServer session, and screen capture is not permitted in this environment).
